...
class modAgent:
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            if len(result) > 0 and result[-1].is_done:
                ...
                await self.send_log(f'📄 Result: {result[-1].extracted_content}')
    ...
    async def _handle_step_error(self, error: Exception) -> list[ActionResult]:
        ...
        if isinstance(error, (ValidationError, ValueError)):
            ...
            if 'Max token limit reached' in error_msg:
                ...
                await self.send_log(f'Cutting tokens from history - new max input tokens: {self.message_manager.max_input_tokens}')
    ...
    async def _log_response(self, response: AgentOutput) -> None:
        ...
        await self.send_log(f'{emoji} Eval: {response.current_state.evaluation_previous_goal}\n🧠 Memory: {response.current_state.memory}\n🎯 Next goal: {response.current_state.next_goal}')
    ...
    async def _log_agent_run(self) -> None:
        ...
        await self.send_log(f'🚀 Starting task: {self.task}')
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        ...
        try:
            ...
            for step in range(max_steps):
                ...
                await self.send_log('❌ Failed to complete task in maximum steps')
            return self.history
    ...
    async def _validate_output(self) -> bool:
        ...
        if not is_valid:
            ...
            await self.send_log(f'❌ Validator decision: {parsed.reason}')
    ...
    async def rerun_history(
        ...
        for i, history_item in enumerate(history.history):
            ...
            await self.send_log(f'Replaying step {i + 1}/{len(history.history)}: goal: {goal}')
        ...
        if images:
            ...
            try:
                with open(output_path, "rb") as file:
                    gif_data = file.read()
                    await self.websocket.send_bytes(gif_data)
            except Exception as e:
