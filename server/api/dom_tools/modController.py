    ...
    def _register_default_actions(self):
        ...
        async def click_element(params: ClickElementAction, browser: BrowserContext):
            ...
            try:
                ...
                await self.send_log(f'Element xpath: {element_node.xpath}')
                if len(session.context.pages) > initial_pages:
                    new_tab_msg = 'New tab opened - switching to it'
                    msg += f' - {new_tab_msg}'
                    logger.info(new_tab_msg)
                    await self.send_log(new_tab_msg)
                ...
                await self.send_log(f'Element no longer available with index {params.index}')
            ...
            try:
                ...
                for locator in locators:
                    try:
                        ...
                        await self.send_log(f'Locator attempt failed: {str(e)}')
                        continue
        ...
        async def get_dropdown_options(index: int, browser: BrowserContext) -> ActionResult:
            ...
            try:
                ...
                for frame in page.frames:
                    try:
                        ...
                        if options:
                            ...
                            await self.send_log(f'Found dropdown in frame {frame_index}')
                            logger.debug(f'Dropdown ID: {options["id"]}, Name: {options["name"]}')
                            await self.send_log(f'Dropdown ID: {options["id"]}, Name: {options["name"]}')
                        ...
                        await self.send_log(f'Frame {frame_index} evaluation failed: {str(frame_e)}')
                    frame_index += 1
                ...
                await self.send_log(f'Failed to get dropdown options: {str(e)}')
        ...
        async def select_dropdown_option(
            ...
            if dom_element.tag_name != 'select':
                ...
                await self.send_log(f'Element is not a select! Tag: {dom_element.tag_name}')
            ...
            await self.send_log(f"Attempting to select '{text}' using xpath: {dom_element.xpath}")
            await self.send_log(f'Element attributes: {dom_element.attributes}')
            await self.send_log(f'Element tag: {dom_element.tag_name}')
            ...
            try:
                ...
                for frame in page.frames:
                    try:
                        ...
                        await self.send_log(f'Trying frame {frame_index} URL: {frame.url}')
                        ...
                        if dropdown_info:
                            if not dropdown_info.get('found'):
                                ...
                                await self.send_log(f'Frame {frame_index} error: {dropdown_info.get("error")}')
                                continue

                            logger.debug(f'Found dropdown in frame {frame_index}: {dropdown_info}')
                            await self.send_log(f'Found dropdown in frame {frame_index}: {dropdown_info}')
                            ...
                            await self.send_log(f'Selection result: {result}')
                            ...
                            if result.get('success'):
                                ...
                                await self.send_log(msg + f' in frame {frame_index}')
                                return ActionResult(extracted_content=msg, include_in_memory=True)
                            else:
                                logger.error(f'Selection failed: {result.get("error")}')
                                await self.send_log(f'Selection failed: {result.get("error")}')
                                if 'availableOptions' in result:
                                    logger.error(f'Available options: {result["availableOptions"]}')
                                    await self.send_log(f'Available options: {result["availableOptions"]}')

                    except Exception as frame_e:
                        logger.error(f'Frame {frame_index} attempt failed: {str(frame_e)}')
                        logger.error(f'Frame type: {type(frame)}')
                        logger.error(f'Frame URL: {frame.url}')
                        await self.send_log(f'Frame {frame_index} attempt failed: {str(frame_e)}')
                        await self.send_log(f'Frame type: {type(frame)}')
                        await self.send_log(f'Frame URL: {frame.url}')
        ...
        for i, action in enumerate(actions):
            if action.get_index() is not None and i != 0:
                ...
                if not new_path_hashes.issubset(cached_path_hashes):
                    ...
                    await self.send_log(f'Something new appeared after action {i} / {len(actions)}')
                    break
